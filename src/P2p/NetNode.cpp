...
#undef INVOKE_HANDLER
  ...
  bool NodeServer::handshake(CryptoNote::LevinProtocol& proto, P2pConnectionContext& context, bool just_take_peerlist) {
    ...
    if (rsp.node_data.version < CryptoNote::P2P_MINIMUM_VERSION) {
      ...
      logger(Logging::WARNING) << context << "COMMAND_HANDSHAKE Warning, your software may be out of date. Please visit: https://pluracoin.org for a new version of software.";
      //todo
    }
  ...
  void NodeServer::banCheckLoop() {
    logger(TRACE) << "Checking IP ban list";
