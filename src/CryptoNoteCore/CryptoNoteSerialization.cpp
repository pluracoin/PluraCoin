...
void serializeBlockHeader(BlockHeader& header, ISerializer& serializer) {
  ...
  if (header.majorVersion == BLOCK_MAJOR_VERSION_2 || header.majorVersion == BLOCK_MAJOR_VERSION_3 || header.majorVersion == BLOCK_MAJOR_VERSION_4) {
    serializer(header.previousBlockHash, "prev_id");
  } else if (header.majorVersion == BLOCK_MAJOR_VERSION_1 || header.majorVersion >= BLOCK_MAJOR_VERSION_5) {
...
if (block.majorVersion >= BLOCK_MAJOR_VERSION_6) {	// todo test version
    serializer(block.signature, "signature");
  }

  if (block.majorVersion == BLOCK_MAJOR_VERSION_2 || block.majorVersion == BLOCK_MAJOR_VERSION_3 || block.majorVersion == BLOCK_MAJOR_VERSION_4) {
