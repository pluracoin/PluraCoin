...
bool Core::check_tx_fee(const Transaction& tx, const Crypto::Hash& txHash, size_t blobSize, tx_verification_context& tvc, uint32_t height) {
  ...
  if (!isFusionTransaction && !m_checkpoints.is_in_checkpoint_zone(height)) {
    ...
    if ((height <= CryptoNote::parameters::UPGRADE_HEIGHT_V3 && fee < CryptoNote::parameters::MINIMUM_FEE_V1) ||
        (height > CryptoNote::parameters::UPGRADE_HEIGHT_V3 && height <= CryptoNote::parameters::UPGRADE_HEIGHT_V4 && fee < CryptoNote::parameters::MINIMUM_FEE_V2))
    {
      enough = false;
    }
